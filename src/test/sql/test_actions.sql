...
v_query text;
BEGIN
CALL @extschema:cuneiform_parser@.parse(E'1\ta--a a-a--a', '@extschema:cuneiform_corpus@', 'sumerian', false, -1);
...
RETURN NEXT is(content, E'1\t%person a a a-a--a') FROM @extschema:cuneiform_serialize_corpus@.transliterations_serialized WHERE transliteration_id = -1;
FOREACH v_query IN ARRAY v_queries LOOP
    DISCARD PLANS;
    RAISE INFO USING MESSAGE = v_query;
    EXECUTE v_query;
