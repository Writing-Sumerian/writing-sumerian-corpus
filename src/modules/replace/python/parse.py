...
def parse_replacement(code: str, id: InOut[int]):
    ...
    corpus_plan = plpy.prepare(
        f"INSERT INTO @extschema@.replace_pattern_corpus VALUES ($1, $2, $3, NULL, NULL, CASE WHEN $5 = 'value' OR $5 = 'sign' THEN NULL ELSE $4 END, $5, $6, $7, $8)", 
        ['integer', 'integer', 'integer', 'text', '@extschema:cuneiform_sign_properties@.sign_type', '@extschema:cuneiform_sign_properties@.indicator_type', 'boolean', 'boolean']
    )

    corpus_unencoded_plan = plpy.prepare(
        f'INSERT INTO @extschema@.replace_pattern_corpus_unencoded VALUES ($1, $2, $3, $4, $5)', 
        ['integer', 'integer', 'text', 'text', '@extschema:cuneiform_sign_properties@.sign_type']
    )

    words_plan = plpy.prepare(
        f'INSERT INTO @extschema@.replace_pattern_words VALUES ($1, $2, $3, $4)',
        ['integer', 'integer', 'integer', 'boolean']
    )

    compounds_plan = plpy.prepare(
        f'INSERT INTO @extschema@.replace_pattern_compounds VALUES ($1, $2, $3, $4)',
        ['integer', 'integer', '@extschema:cuneiform_sign_properties@.pn_type', '@extschema:cuneiform_sign_properties@.language']
    )
    ...
    plpy.execute(f"""
        UPDATE @extschema@.replace_pattern_corpus SET 
            value_id = a.value_id, 
            sign_variant_id = a.sign_variant_id 
        FROM 
            @extschema@.replace_pattern_corpus_encoder a 
        WHERE 
